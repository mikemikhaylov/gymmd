...
| Workout | `workouts/active/` or `workouts/completed/` | `workout_id` (e.g. `wkt-a91f3`) |
...
- **Workout `status` drives folder placement** — `in_progress` → `active/`, `completed` → `completed/`, moved via `fileManager.renameFile()`. No abandon flow; discard = delete the file.
...
4. ✅ Workout creation from template (`main.startWorkout`, `services/workout-store.ts`); one active at a time
