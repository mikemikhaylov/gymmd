...
- **`current_phase_started`**: the single timestamp the live stopwatch needs. Only meaningful while `in_progress`; it is **omitted from the file entirely once cleared** (so completed workouts never carry it). On plugin load, if a workout has `status: in_progress`, the timer is recomputed as `Date.now() - current_phase_started` — not from a running interval, so it's correct immediately after reopening Obsidian regardless of how long it was closed.
...
- **Recording-conventions note.** The generated body includes a short blockquote just under the title (`WORKOUT_NOTE`) clarifying that weights are the total load (dumbbells = both combined) and reps are total reps (alternating/both-side exercises = both sides combined). It's part of the write-only body, so it never affects parsing; it just makes each workout self-explanatory for a human/AI coach. Per-exercise specifics also live in each exercise file's notes.
...
**Mobile keyboard:** in the **Active Workout** view, reps/weight are edited via a **tap-to-open number popup** (`NumberCell` → `promptNumber` modal) rather than inline inputs. Obsidian modals handle the on-screen keyboard correctly (the inline approach had the keyboard cover/clip the bottom-pinned fields on Android). The popup carries the same validation (reps integer 1–999; weight ≥0, ≤2 decimals). The template editor (itself a modal) keeps inline `NumberField` inputs, which work fine there.
...
- **Rest-timer background tint:** while resting (no active set), the whole view background stays normal for the first **1:30**, then reddens a **clearly glanceable** step every **30s** (~19% red at 1:30 → ~68% at 5:00, then capped). It returns to normal the moment a set starts — readable from across the gym with the phone on a bench.
